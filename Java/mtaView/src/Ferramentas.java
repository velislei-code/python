...
public class Ferramentas {
	...
	public int ContarReg(JTable jtTabela){
		...
		int iL = 0;
		boolean bContem6 = true;
	    while (bContem6) {
	    	//if(jtTabela.getValueAt(iL, iC) != ""){	iContaReg = iL;	}
			if(jtTabela.getValueAt(iL, 0).toString().contains("6")){	
				bContem6 = true;
				iContaReg++;	
			}else{
				bContem6 = false;
...
//		objCxD.Aviso("Total de linhas na tabela: " + iContaReg, true);
	...
	public String AnalisarTeste(int iLinMdX, String sColAcao){
 		
 		// faz uma análise dos resultados dos testes, informa na celula: Ação
 		objLog.Metodo("Ferramentas().AnalisarTeste()");
 		
 		String sRes = sColAcao;
 			
 			// Verifica se o testes finalizou com Testado
 			if(sColAcao == objDef.AcaoFimTst){
 				// Informa com testes OK
 				sRes = objDef.AcaoFimTstOK;
        
 			}else{
 				// Informa falha no teste
 				sRes = objDef.AcaoFimTstNOK;
        
 				/*
 				// Verifica se o testes finalizou com Simulado
	 			if(sColAcao == objDef.AcaoFimSim){
	 				// Informa com testes OK
	 				sRes = objDef.AcaoFimSimOK;
	        
	 			}else{
	 				// Informa falha no teste
	 				sRes = objDef.AcaoFimSimNOK;
	...
	public void LimparTabela(JTable jtTabela){
		...
		objLog.Metodo("mtaView().LimparTabela()");
		
		int iNumLinTab = jtTabela.getRowCount();	
	...
	public boolean CheckTestar(String sCelula){
		
		// Se celula Acao contem indicador "Testar"
		if( !sCelula.contains(objDef.AcaoTestar) ){
			return true;
		}else{
			return false;
		}
