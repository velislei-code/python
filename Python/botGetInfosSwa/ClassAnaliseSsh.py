...
class AnaliseSsh:
    ...
    def analisePendencias(self, caminho_arquivo, pwLog, icoAppGetInfosSwaX, icoAppGetInfosSwaY, pagMobaX, pagMobaY, posIniArrasteCopyX, posIniArrasteCopyY, janMobaX, janMobaY, abaMoba2X, abaMoba2Y, abaMoba3X, abaMoba3Y, nEnter, tFast, tCmd, tLogar, desviaBugsLogar, desviaBugsCmd):
        ...
        try:    # Check-ok
            ...
            print(f"Exception analisePendencias() - Erro ao ler arquivo {e}")
            objTools.debbug(myIdx.noJumpLin, f"Exception objAnaliseSsh.analisePendencias() -  {e}")
            # return None
        
    # Final def analisePendencias    

    def analiseDados(self, call, origem, modeloEDD, procurar, Dados, IpdOrVpn):
        objTools.debbug(myIdx.noJumpLin, 'objAnaliseSsh().analiseDados('+origem+', ' +str(procurar)+', '+Dados+')')
        ...
        try:
            ...
            print(f"Exception objAnaliseSsh.analiseDados() -  {e}")
            objTools.debbug(myIdx.noJumpLin, f"Exception objAnaliseSsh.analiseDados() -  {e}")
         

    # ------------ Final analiseDados-------------------------------------------------------------------------------------------------------------------- #
    # end def analiseDados
    ...
    #   readInfosSWA(myIdx.USER, varGbl_NomeSWA,       varGblNomeSwa,   csvIdOrdem, csvProduto, posIniArrasteCopyX, posIniArrasteCopyY, myIdx.UND,      desviaBugsCmd, tCmd)  
        ...
        objTools.debbug(myIdx.noJumpLin, 'objAnaliseSsh().readInfosSWA(myIdx.USER, '+IpNomeSwaGetCatDevice+', '+ nomeSwaGetQuery+', '+ csvIdOrdem+', '+ Produto+', '+ str(posIniArrasteCopyX)+', '+  str(posIniArrasteCopyY)+', '+ str(tipoCheck)+', '+ str(desviaBugs)+', '+   str(tCmd)+')')
        ...
        try:
            ...
            if call == myIdx.USER:     # FUNCAO CHAMADA VIA MENU(Auto/manual)
                objTools.debbug(myIdx.noJumpLin, 'objAnaliseSsh().readInfosSWA(myIdx.USER, Fechando ´}` com gravar: } )')
                objArquivos.gravar_infosArqTransferPhp(myIdx.infosSwaPyToPhp, myIdx.ADD, '\n}\n')   # limpar arquivo txt de Transfers dados  To Php
                #objArquivos.gravar_infosArqTransferPhp(myIdx.infosSwaPyToPhp, myIdx.ADD, '\n===========================================================================================\n')   
                objTools.debbug(myIdx.noJumpLin, 'objAnaliseSsh().readInfosSWA(myIdx.USER, Fechou ´}` )')
            ...
            print(f"Exception objAnaliseSsh.readInfosSWA() -  {e}")
            objTools.debbug(myIdx.noJumpLin, f"Exception objAnaliseSsh.readInfosSWA() -  {e}")
        

    # end def readInfosSWA
    ...
    def printScreen(self, posIniArrasteCopyX, posIniArrasteCopyY, tCmd):
        ...
        objTools.debbug(myIdx.noJumpLin, 'objAnaliseSsh().printScreen()')
        ...
        try: 
            ...
            print(f"Exception objAnaliseSsh.printScreen() -  {e}")
            objTools.debbug(myIdx.noJumpLin, f"Exception objAnaliseSsh.printScreen() -  {e}")
    ...
    def readInfosRSD(self, call, localizar, nomeRAgetQuery, numPtRAgetQuery, nomeHL5GQuery, nomeSVlanQuery, IPDorVPN, posIniArrasteCopyX, posIniArrasteCopyY, desviaBugs, tCmd):
        ...
        try:
            objTools.debbug(myIdx.noJumpLin, 'objAnaliseSsh().readInfosRSD('+str(desviaBugs)+')')
            ...
            print(f"Exception objAnaliseSsh.readInfosRSD() -  {e}")
            objTools.debbug(myIdx.noJumpLin, f"Exception objAnaliseSsh.readInfosRSD() -  {e}")
        
         
        
    # end def readInfosRSD
    # -------------------------------------------------------------------------------------------------------------------------------- #
    def catDeviceList(self, call, ufSwaX, nomeSwaX, folhaMobaX, folhaMobaY, posIniArrasteCopyX, posIniArrasteCopyY, tFast):
        ...
        try:
            ...
            print(f"Exception objAnaliseSsh.catDeviceList() -  {e}")
            objTools.debbug(myIdx.noJumpLin, f"Exception objAnaliseSsh.catDeviceList() -  {e}")
               
    # end def catDeviceList
