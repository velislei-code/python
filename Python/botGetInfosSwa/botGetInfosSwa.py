import sys
import traceback    # Debbug de erros(try-excption) -0 so encerra após exibir erro
import time
import configparser
import pyperclip
...
from ClassArquivos import Arquivos  
from ClassAnaliseSsh import AnaliseSsh  
from ClassWindow import Windows  
from ClassTools import Tools  
from ClassBot import Bots  
from ClassMobaTerm import Terminal  
...
def main():
    ...
    # carregar config posições
    vtCFG = objArquivos.lerConfigIni()
    
    tFast = float(vtCFG[myIdx.cfgTFast])
    tCmd = float(vtCFG[myIdx.cfgtCmd])
    tLogar = float(vtCFG[myIdx.cfgtLog])

    desviaBugsLogar = int(vtCFG[myIdx.cfgDsvBugLog]) 
    desviaBugsCmd = int(vtCFG[myIdx.cfgDsvBugCmd]) 
    icoAppGetInfosSwaX = int(vtCFG[myIdx.cfgIcoAppX]) 
    icoAppGetInfosSwaY = int(vtCFG[myIdx.cfgIcoAppY]) 
    pwLog = str(vtCFG[myIdx.cfgPwLog]) 

    icoMobaX = int(vtCFG[myIdx.cfgIcoAppX]) 
    icoMobaY = int(vtCFG[myIdx.cfgIcoAppY]) 
    janMobaX = int(vtCFG[myIdx.cfgJanMobaX]) 
    janMobaY = int(vtCFG[myIdx.cfgJanMobaY]) 
    abaMoba2X = int(vtCFG[myIdx.cfgAbaMoba2X]) 
    abaMoba2Y = int(vtCFG[myIdx.cfgAbaMoba2Y]) 
    abaMoba3X = int(vtCFG[myIdx.cfgAbaMoba3X]) 
    abaMoba3Y = int(vtCFG[myIdx.cfgAbaMoba3Y]) 
    folhaMobaX = int(vtCFG[myIdx.cfgPagMobaX]) 
    folhaMobaY = int(vtCFG[myIdx.cfgPagMobaY]) 
    nEnter = int(vtCFG[myIdx.cfgnEnter]) 

    # Posições X,Y quadro Copiar texto-teste-IP-Moba(Copy&arraste)
    posIniArrasteCopyX = int(vtCFG[myIdx.cfgPosIniArtCpX]) 
    posIniArrasteCopyY = int(vtCFG[myIdx.cfgPosIniArtCpY]) 

    # Pagina-Php
    PhpTaRascunhoX = int(vtCFG[myIdx.cfgPhpTaRscX]) 
    PhpTaRascunhoY = int(vtCFG[myIdx.cfgPhpTaRscY]) 
    PhpBtnSalvarX = int(vtCFG[myIdx.cfgPhpBtnSvX]) 
    PhpBtnSalvarY = int(vtCFG[myIdx.cfgPhpBtnSvY])
    
    
    # Inicializa vars Globais(Main)
    varGbl_NomeIpSWA = ""
    varGblID =  ""
    varGblIpdVpn =  ""
    varGblModEDD =  ""                 
    ...
    while True: 
        ...
        # except Exception:
            ...
            if stp == 'S' or stp == 's': 
                ...
                # Tenta recarregar de *.ini
                varGblNomeSwa = objArquivos.lerVarsIni(myIdx.iniNmSWA) 
                varGblNomeRa = objArquivos.lerVarsIni(myIdx.iniNmRA) 
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Isso garantirá que qualquer exceção seja exibida e o programa só feche após você pressionar Enter.
