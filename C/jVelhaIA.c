...
bool debbug = true;      // My rotine of debbug to build
int iL[N], iC[N], iX[N]; // My rotine of print_debbug to build

char cPlayer = 'X';
char cComputer = 'O';

int numGame = 0; // Count num of Games
int numMove = 0; // Count num Moviments in The game

// [numGame-0a100][nMovimento-0a9][i][j] = -1(ocupped PC), 0(Free) or 1(ocupped Player) cell
int IA_learnMove[Cem][maxMove][maxMove][maxMove]; // Memory move address[x][i][j] = -1(Ocupped PC), 0(Free) or 1(ocupped Player) cell
int IA_learnResult[Cem];                          // Memory result of the moviments of the game
...
void analisys_IA()
{
    // Estatisticas de move dos ganhos do Player
    // Pegue todos moviemntos onde player ganhou
    // faça uma estatística deles
    // mova na posição contraria, q esta livre

    // Pegue todos moviemntos onde computador ganhou
    // faça uma estatistica deles

    for (int g = 0; g <= numGame; g++)
    {
        if (IA_learnResult[g] == PlayerWin)
        {
            printf("IA[g%i] = %i\n", g, IA_learnResult[g]);
            for (int i = 0; i < N; i++)
            {
                for (int j = 0; j < N; j++)
                {
                    if (IA_learnMove[g][numMove][i][j] == Player)
                    {
                        IA_learnMove[g][numMove][i][j];
                        printf("analisys()->IA[g%i][m%i][i%i][j%i] = %i, ", g, numMove, i, j, IA_learnMove[g][numMove][i][j]);
                        printf("\n");
                    }
                }
            }
        ...
        for (int g = 0; g <= numGame; g++)
        {
            for (int m = 0; m < maxMove; m++)
            {
                for (int i = 0; i < N; i++)
                {
                    for (int j = 0; j < N; j++)
                    {
                        if (IA_learnMove[g][m][i][j] != 0)
                        {
                            printf("IA[g%i][m%i][i%i][j%i] = %i, ", g, m, i, j, IA_learnMove[g][m][i][j]);
                            printf("\n");
                        }
                    }
                }
...
void restart(void)
{ // Reset counts of the game
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < N; j++)
        {
            iJv[i][j] = Free; // All positions Free
            cJv[i][j] = ' ';  // Clear in all positions
        }
    }
    numGame++;   // Increment num Play
    numMove = 0; // reset
    render();
    return;
}
...
bool save_move(int i, int j, char cXorO, char cOrigin)
{
    ...
    if (iJv[i][j] == Free) // If positions is free...
    {
        ...
        IA_learnMove[numGame][numMove][i][j] = Computer; // Memory moviment of Computer to analisys future
        numMove++;                                       // Count number Moviments executeds
...
bool game_over(void)
{
    ...
    */
    ...
    if (iResultWin != 0)
    {
        ...
        IA_learnResult[numGame] = iResultWin; // Memory data for analisys future
...
bool prepare_move(void)
{
    ...
    */
    ...
    int wa = 2;
    int wb = 2;
    // if position(privelegied) is free(X -> W, W -> X)
...
int main(void)
{
    ...
    do
    {
        ...
        if (iJv[i][j] == Free)
        {
            ...
            render();
            IA_learnMove[numGame][numMove][i][j] = Player; // Memory moviment of Player to analisys future
            numMove++;                                     // Count number moviments executeds
