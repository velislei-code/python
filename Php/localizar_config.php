...
<div id="geral"><!-- Pagina Geral -->	
	...
	<form name="LocalizarConfig" method="post" action="localizar_config.php?<?Php echo "topico=".$TopicoURL; ?>"><!-- Form Localizar, inserido aqui devido espa�os que cria no IE -->
...
<?Php
			...
			if(isset($_POST['BtRfnPesquisa'])){	
				//echo 'PesqConfig = ObjTickets->LocalizarRefinar('.$CxCampoX.', '.$edCpoPesquisaX.')<br>';	
				//$PesqConfig = $ObjTickets->LocalizarRefinar($CxCampoX, $edCpoPesquisaX, $CxCampo2Y, $edCpoPesquisa2Y, $edCpoDataX);				
                echo 'Em construção!';
                
			}else{
				$PesqConfig = $ObjConfigRouter->LocalizarSimples($PesquisarX);
			}
				
           	if($PesqConfig[_CtgVETOR][_CtgVETOR] > 0){ // Confirma se consulta retornou algum registro		
				
				
                /*
				// Ajuste para n�o imprimir registro vazio
				if($PesqConfig[0][2] == ""){
					$TotalP = count($PesqConfig)-2;
...
?>
				...
				<tr><td>
					...
					<?Php
						...
						echo 'Sua busca retornou: '.$PesqConfig[_CtgVETOR][_CtgVETOR].' registros.'.'<br>';
						echo '</b><font size="1" face="Verdana" color="#008000" font-weight: normal;>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
						echo "     SELECT * FROM rav WHERE ".$CxCampoX." LIKE '%".$edCpoPesquisaX."%' AND ".$CxCampo2Y." NOT LIKE '%".$edCpoPesquisa2Y."%' AND data LIKE '%".$edCpoDataX."%' ORDER BY registro DESC";
     	
						for($E = 0; $E < $PesqConfig[_CtgVETOR][_CtgVETOR]; $E++){
						$Eb = $E+1;								
					...
					?>						
						<tr>
							<td width="3%" colspan="1" height="20" align="left" valign="top"></td>
							<td width="94%" colspan="1" height="20" align="left" valign="top">
								<!-- <a href="<?Php printf("%s?%s&*reg=%d",$AttribLinkAbaIntB[2],$PesqConfig[$E][_TIPO],$PesqConfig[$E][_ROUTER]); ?>" class="fonte_lista">-->
								<a href="<?Php printf("config_router.php?id=%d",$PesqConfig[$E][_REG]); ?>" class="fonte_lista">
									<?Php printf("%d - %s",$Eb,$PesqConfig[$E][_REG]); ?>
									<i><?Php printf("(%s, %s)-[ %d ], %s ",$PesqConfig[$E][_REG], $PesqConfig[$E][_TIPO], $PesqConfig[$E][_ROUTER], $PesqConfig[$E][_Data]); ?></i>
								</a>
							</td>
						...
						<tr>
							...
							<td width="500" colspan="1" height="20" align="left" valign="top"><font color="#000000"><font size="2" face="Verdana">							
								<table class="TAB_ConteudoPsq" width=80% align="center" valign="top"> 	
									<tr><td>										
										<?Php 
                                            $ParteRascunho = substr($PesqConfig[$E][_CONFIG],0,350); echo"$ParteRascunho..."."<br>"; printf("[%s]", $PesqConfig[$E][_ROUTER]); 
	...
	} // if(!empty($PesqConfig)){ // Confirma se consulta retornou algum registro ?>
	...
	<!-------------------------------- Final Geral Direito -------------------------------------------------------------------->
	<?Php if(!empty($PesqConfig[0][0])){ // Confirma se consulta retornou algum registro -- Bloqueia Rodapé p/ não subir na Tabela ?>
